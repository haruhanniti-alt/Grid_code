...
def backtest(df, stake, fee, grid_min, grid_max, grid_levels,
    ...
    for ts, row in df.iterrows():
        ...
        # ── Pass 1: SELL ──
        # ขายเมื่อ high แตะหรือเกิน sell_level (รองรับราคากระโดดข้าม/gap up)
        #   ปกติ: ราคาแตะ sell_level → ขายที่ sell_level
        #   gap up: แท่งเปิดสูงกว่า sell_level ไปแล้ว (op > sell_level) → ขายที่ราคา open
        #           (จุดแรกที่เทรดได้จริง — ไม่ถือไม้ค้างจน unrealized เพี้ยน)
        for i in range(grid_levels - 1):
            sell_level = levels[i + 1]
            if slots[i] and hi >= sell_level:
                exec_price = max(sell_level, op)   # gap up → ขายที่ open
...
def run_pair(config):
    ...
    # ── Auto price range จาก open แท่งแรก ──
    #   รองรับ 2 แบบ:
    #   (1) range_low_pct + range_high_pct  → ไม่สมมาตร เช่น -50%/+100%
    #       grid_min = open*(1-range_low_pct), grid_max = open*(1+range_high_pct)
    #   (2) range_pct (เดิม)                → สมมาตร ±range_pct
    if 'range_low_pct' in config or 'range_high_pct' in config:
        ref_open = float(df['open'].iloc[0])
        low_pct  = config.get('range_low_pct', 0.50)
        high_pct = config.get('range_high_pct', 1.00)
        grid_min = ref_open * (1 - low_pct)
        grid_max = ref_open * (1 + high_pct)
        print(f"  Auto range: open={ref_open:,.2f} → "
              f"grid {grid_min:,.2f} – {grid_max:,.2f} "
              f"(-{low_pct*100:.0f}% / +{high_pct*100:.0f}%)")
    elif 'range_pct' in config:
        ref_open = float(df['open'].iloc[0])
        rp = config['range_pct']
        grid_min = ref_open * (1 - rp)
        grid_max = ref_open * (1 + rp)
        print(f"  Auto range: open={ref_open:,.2f} → "
              f"grid {grid_min:,.2f} – {grid_max:,.2f} (±{rp*100:.0f}%)")
    else:
        grid_min = config['grid_min']
        grid_max = config['grid_max']
    grid_levels = config['grid_levels']
    print(f"  Range: {grid_min:,.2f} → {grid_max:,.2f} {config.get('currency','')}")
    print(f"  Stake: {config['stake']:,} {config.get('currency','')}  ·  Grids: {grid_levels}")

    records = []
    for fee in config['fees']:
        print(f"\n  Running fee={fee*100:.3f}% ...")
        res = backtest(df, config['stake'], fee,
                       grid_min, grid_max, grid_levels,
                       config.get('slippage', SLIPPAGE))
        res['pair'] = config['pair']
        res['currency'] = config.get('currency', 'USDT')
        res['source'] = config.get('source', '')
        res['grid_tag'] = config.get('grid_tag', str(grid_levels))
        res['year_tag'] = config.get('year_tag', '')
        # เก็บข้อมูล range ที่ใช้จริง (ให้ dashboard แสดงป้าย Price Range ถูก)
        if 'range_low_pct' in config or 'range_high_pct' in config:
            res['range_low_pct']  = config.get('range_low_pct', 0.50)
            res['range_high_pct'] = config.get('range_high_pct', 1.00)
        elif 'range_pct' in config:
            res['range_low_pct']  = config['range_pct']
            res['range_high_pct'] = config['range_pct']

        records.append({
            "fee": fee, "stake": config['stake'],
            "currency": config.get('currency', 'USDT'),
            "source": config.get('source', ''),
            "grid_tag": config.get('grid_tag', str(grid_levels)),
            "year_tag": config.get('year_tag', ''),
        ...
        print(f"    Trades={res['total_trades']}  APY={res['apy']}%  "
              ...
              f"Slots={res['coin_slots_held']}/{grid_levels}")
